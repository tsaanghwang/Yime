...
- 四宿主（记事本、Word x64；Firefox、Notepad++ x86）的三模式、各三轮行为结果已由 [09-11 多宿主报告](docs/project/YIMECORE_TEST_PC_MULTI_HOST_REPORT_2026-09-11.md)记录为用户人工通过，并附对应 DLL 元数据。
- 本次重启登录自启动已附独立注册表、当前进程与 Shell-Core 9708 证据；不再列为缺少本次重启证据。
- 历史注册/默认输入法前后保护对照暂缺，按用户决定暂缓；更长时间日常使用与 L5/L6 完整封存仍待完成。
- 新非法码元拒绝修复没有安装到此被测包，不能继承本次安装态结果。
- ARM64 原生构建或执行、签名和正式发布。

因此 `local_product_ready=false`、`public_release_ready=false`；当前安装、注册宿主、重启和四宿主行为通过，不等于完整保护/维护矩阵、长期使用或正式发布完成。
