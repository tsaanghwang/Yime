...
BCC 中出现但没有整串进入静态词库的材料，如果能够递归拆成已有来源合格读音、并已通过正式音元链编码的字或短语，原则上可以依靠连续编码、分段选词和 YimeCore 递归组合完成输入，不要求把整句预先收录为候选。这个能力有望显著扩大实际句子覆盖，同时减少为了覆盖长句而持续扩张静态词库的压力。

误差边界必须保留：BCC 只证明字串及其频次，不提供整串读音；组件拼音的拼接只是组合输入路径，不能据此建立新的规范词音。离线“可达”也不等于运行时一定出现、排在首选或无需纠错；多音字、同音候选、切分竞争、组件未进入运行索引以及缺少单字编码基础，都可能造成候选偏差或不可达。只有三种编码模式的真实 YimeCore 回放同时达到较高可达率、较低纠错次数和稳定提交，才能把这项潜在能力评价为良好运行效果。

每日抽样回放进一步把失败区分为运行索引图缺边、production beam 淘汰、路径保留但排序落后和输入无效，并以 `developer_alerts.json` 提示开发者调查相应层级。该报告使用无用户模型实例；BCC 目标不得自动写入静态词典、用户正负反馈或可选线性重排器，否则会把评测集泄漏为训练集。
