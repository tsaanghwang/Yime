...
这些检查不代替真实安装、输入和重启验收。包含 PR #57 的完整双产品包、来源证明和本地验证范围见 [HANDOFF](HANDOFF.md)；本轮仅复核既有交付，不发起安装或维护，不能将旧包通过记录当作新包验收。
...
2026-09-14 的历史维护包/源码候选包已在各自报告范围内通过本机两套卸载、重装、文件哈希、x64/x86 注册及运行路径检查，用户确认重启前后两套输入正常。测试端的单套、双套和交叉维护保留占用后重启处理条件，详见 [验证记录](VALIDATION.md)。这些历史结果不属于 PR #57 完整包，该包尚无实机验收；当前任务见 [本轮交接](HANDOFF.md)。
