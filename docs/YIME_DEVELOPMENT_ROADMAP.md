...
| 5.5 | 分段选词、句中改选与动态组句纠错 | 整句尚未提交时允许定位已组合分段、重新显示该段同音候选并局部改选；保留后续分段，重新生成整句候选，并把人工纠正纳入用户学习。例如将“姻缘输入法”中的“姻缘”改为“音元”，而不必删除并重输整句 | 先核查 librime 已提供的 composition/segment 能力，再补齐 Go 协议、PIME 候选窗、按键交互和学习状态的端到端映射；不得破坏现有分页权和 Base 数字编码 |
