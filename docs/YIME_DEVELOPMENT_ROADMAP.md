...
Phase 5 原则上不属于 1.4.0 发布收口范围；其中不改变选词行为的 5.1 候选标签修正已提前完成。候选排序微调、非标准音节审计和数字键模式尤其需要先形成数据口径、交互约束、兼容性方案和验收基线；在专项规划获批前不得顺手修改实现或词典数据。

| # | 任务 | 说明 | 前置条件 |
|---|------|------|----------|
| 5.1 | 候选窗标签与选择键一致 | ✅ 新增 `setSelLabels`，显示 `⇧1`—`⇧9`；保留 `SetSelKeys` 向后兼容 | 无 |
