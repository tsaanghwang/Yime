...
> 更新日期：2026-09-07
> 当前版本：`1.4.0-dev.1`（正式目标仍为尚未发布的 `v1.4.0`）
...
- PIME/TSF 宿主、Rust 启动器、Go 后端和原生工具均已进入构建链；安装维护使用 [installer/simple](../installer/simple/README.md)；
...
| 开发版安装 | 前一身份已复核 | 2026-07-31 的重启替换与哈希一致结论属于 `1.4.0-dev`；当前 `1.4.0-dev.1` 尚未安装，也不继承该结论 |
...
2026-07-31 的 `main`（`58b3fd7d`）曾生成状态为 `complete` 的安装态报告：
三个宿主均为 `pass`，三项安装哈希均匹配，6 组
`selectCompositionSegment` 事务全部按 `client`、`seqNum` 关联到响应。阶段 A1
只剩第一段、中间段、末段反复切换的观察期结论。该报告现作为历史基线保留；当前开发版应以
最近一次完整安装态验证及对应专项验收记录作为滚动基线，不再把旧提交写成“当前”构建。
...
| 普通话语流音变处理 | 轻声阶段 3-2 仍停留在 74,535 条三模式临时包；词汇化儿化显式来源试点的 131 条记录均已完成融合投影（65 条核心权重、66 条 PSC 外围低权重），每模式 196 条正式附加路线。阶段 4A 登记 18 个派生音元：中央卷舌音元复用 3 个共享键位类；`a˞`、`ɤ˞`、`ã˞`、`ɤ̃˞`、`u˞` 分别试用 `F/D/S`、`R/E/W`、`Y/T/Q`、`V/C/X`、`P/A/Z` Shift 层；阶段 4B 已为 131 条融合路线生成可解释反查旁表 | 对新增三组试行键位及 `ie/üe`、`ian/üan`、`uo`、`ao/ou/o/u` 规则做安装态验证。轻声、能产儿化和未获显式来源授权的词条不得借此试点自动进入运行时 |
...
1. 以最近一次 `Installed runtime verification: complete` 及对应专项验收记录作为滚动观察基线；
2. 先完成运行数据刷新、Rime 导入词典编译新鲜度和驻留进程退出结果的闭环加固；
3. 继续覆盖第一段、中间段、末段反复切换及长时间宿主运行；
4. 观察期只处理阻断性缺陷、证据缺口和文档漂移；语料不足的阶段 7 只做离线准备；
5. 等待可信签名条件，不把 unsigned pre-release 当作正式发行；
6. 取得签名条件后生成新的签名构建和清单，并重复安装、三宿主、语言栏菜单、CodeIntegrity
   和卸载验收。
...
| vendored 构建依赖缺失或被修改 | CMake/Cargo 离线解析或哈希门禁失败 | 恢复锁定的 Corrosion/crate 快照并运行 vendored 依赖验证器 |
