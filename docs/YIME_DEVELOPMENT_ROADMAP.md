...
> 单一等长真源仍是离线来源与派生边界；Windows 默认发布层已经切换为精简核心词库。实施结果见
> [从三套码表回到一个真源](project/SINGLE_SOURCE_LEXICON_REFACTOR.md)，原阶段设计见
> [单一等长真源码表阶段准备](project/SINGLE_CANONICAL_LEXICON_PREPARATION.md)。
> 三套旧 Rime 大词库保留为离线真源、筛选池和回归资源，不再进入安装运行包。

> 版本：2026-07-27（默认动态组句词库与纯净用户态学习闭环）
> 实施分支：`codex/core-trial-lexicon`
...
| 40 | 完整大词库 handoff | 历史能力保留 | 原型可导出并派生 2,456,797 条三模式词典；现仅作离线真源、筛选和回归 |
| 41 | 默认核心词库发布 | 当前实现 | `yime_core_trial` 的 1,124,631 条已编码组件成为默认；旧三套大词库退出安装运行包 |
| 42 | 动态组句与学习闭环 | 当前实现 | 未预装长词可由组件生成；一次纠正后成为首选，重启后保持 |
| 43 | 运行包噪声隔离 | 当前实现 | 构建删除旧 dict/schema/manifest；安装验证把任何遗留文件报告为 `runtime-leak` |
...
- 等长大词典是离线唯一可维护真源；默认安装只消费版本化核心词典，旧变长、等长、省键运行产物
  不得作为安装回退；
...
- 默认方案必须保持 `yime_core_trial`；无配置或遗留方案在共享 schema 已退出时迁移到该方案。
...
4. 对安装包持续执行旧大词库泄漏门禁、反查核心词库优先级和全新用户目录部署测试；
...
| 4.3 | 词典噪声清理 | 待做 | 按统一候选整理流程审查无 BCC 证据及低频候选；频率只安排审查顺序，不直接代替词汇性判断 |
