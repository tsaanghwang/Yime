...
| 普通话语流音变处理 | 轻声阶段 3-2 仍停留在 74,535 条三模式临时包；词汇化儿化显式来源试点共有 131 条记录，29 条双路线就绪，15 条继承三模式权重并生成 90 行正式附加词典；阶段 4A 用 9 个派生音元、3 个共享键位类和现有布局载体完成重投影，阶段 4B 已为 15 条实际运行融合路线生成可解释反查旁表，均未扩大运行词条 | 补查 14 条缺权重记录的来源，随后逐类审定 102 条待决融合记录。轻声、能产儿化和鼻化卷舌研究类不得借此试点自动进入运行时 |
