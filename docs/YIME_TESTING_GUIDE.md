...
- 2026-07-24：候选窗独立组句分段条完成安装和重启验证；已安装 `server.exe` 与构建物 SHA-256 一致，x86/x64 `PIMETextService.dll` 也分别一致。Notepad、Codex IDE 已完成初步试用，确认功能可见并有实际作用；由于鼠标分段改选需要长期使用评价，本轮记录为“进入观察期”，不替代真实 x86 宿主和重复第一/中间/末段切换验收。
