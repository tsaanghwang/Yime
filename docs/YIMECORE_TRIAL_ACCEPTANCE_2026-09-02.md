...
本轮修复了英文模式下 `Shift+字母/符号` 后误切回中文的问题。最终 Rime 对照的功能正确性通过，但相对延迟和内存门槛失败；当前 AMD64 机器也不能替代真实 ARM64 Windows 桌面宿主。因此不得启动 E7，也不得宣称已经可以独立替换生产 Rime/PIME。
...
E1 至 E4 的候选、组句、学习、遗忘、持久化和四模块 17,622 条路径正确性通过。相对 Rime 的性能/内存门槛失败，属于当前真实发布阻塞：
...
证据位于 `.tmp/yimecore-experiment/final-rime-compare/`。因此“功能能用”不等于“已达到替换 Rime 的发布条件”。
