...
当前可用性存在 **2 个高优先级问题**（设置变更丢失输入、候选选择键不足）和 **5 个中优先级问题**。重复按键吞键（`edd6e0ab`）和回车键被吞问题已修复。
...
**根因**：音元编码体系使用了 57 个音元字符（含零声母分化），映射到 QWERTY 键盘的 47 个可打印键位上。数字键 0-9 全部用于编码（如 zh=7, ch=8, sh=9, j=3, q=2, x=1, z=6, c=5, s=4, r=0, w=%, y=$），字母键也全部属于 alphabet，标点符号键多数也在 alphabet 中。在 57 个音元占满键位的前提下，无法再分配键位给候选选择。
...
**为何暂不修改**：改变选择键方案需要重新分配键位，重新分配键位需要修改编码体系，修改编码体系需要重建原型导入流程和全部词典，等同于重建项目。当前方案是在编码约束下的最优折中。
