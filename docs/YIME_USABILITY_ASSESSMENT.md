...
当前实现已扩展到 9 个 Shift+数字候选键，并以带 Shift 提示的标签解决可发现性问题；下文旧的“五候选键”结论仅作历史记录。
...
- 旧布局候选选择键仅 5 个，第 6-9 个候选需鼠标点击
- Base 数字键在组字时作为编码输入，不选词
- 旧实现用裸数字 `SetSelKeys` 作显示标签，容易让人误以为数字键可直接选词

### 可接受的后续改进（不涉及编码变更）

- ✅ 通过 `setSelLabels` 显示 `⇧1`…`⇧9`；`SetSelKeys` 只保留作旧宿主兼容
