...
| 词典质量 | ★★★★☆ | 规模大、权重分层合理；无 BCC 频率的词语默认权重=1（有意设计，非噪声） |
...
- **无 BCC 频率的词语**：默认权重=1（`PHRASE_LEXICON_DEFAULT_FREQUENCY`），表示"至少被收录一次"。这些词在候选列表中始终排在有频率数据的词之后，不影响正常使用。weight=1 之间的排序是随机的，但它们都在列表底部
...
- 词语字频回退排序（无 BCC 词频时用组成字频率估计，改善 weight=1 词语间排序）
