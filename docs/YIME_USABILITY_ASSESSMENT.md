...
> 产品原则：与流行拼音输入法不同，Yime 不采用裸数字键选词。Base 数字键始终参与音元编码，按序号选词固定使用 Shift+1…Shift+9；不再把“数字键选词模式”列为未来任务。
...
当前实现已扩展到 9 个 Shift+数字候选键，并以带 Shift 提示的标签解决可发现性问题。裸数字键保留为编码是有意的产品设计；下文旧的“五候选键”结论仅作历史记录。
...
| 候选窗交互 | ★★★★☆ | `⇧1`…`⇧9` 明示 Shift 组合；Base 数字仍用于编码 |
...
### 5.4 候选选择键仅5个（历史编码约束，已由试验布局解决）

旧布局中，57 音元（24 首音 + 33 乐音）占满 47 个可打印键位，候选选择键仅有 Space/`/-/=/\ 五个。当前布局已用 Shift+1…Shift+9 解决候选序号选择，同时保持 Base 数字键参与编码；本段只解释旧结论，不构成恢复裸数字键选词的待办。
...
57 音元编码体系中，24 个首音使用数字行 Base 键位，数字键本身就是编码的一部分。与流行拼音输入法不同，Yime 不在候选出现后把裸数字临时改作选词键，也不提供这种切换模式；按序号选词固定使用 Shift+1…Shift+9。这样同一按键在整个组字过程中语义不漂移。
