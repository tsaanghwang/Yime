...
当前状态（2026-09-03 08:56）：local.4 已安装，纠正后的原生安装后验收通过；本页早先“未安装/暂停”的文字为历史诊断。当前证据与剩余门禁见 [local.4 原生升级记录](YIMECORE_LOCAL_PRODUCT_LOCAL4_2026-09-03.md)。不要重复执行本页的首次升级入口。

**08:12 更新：`.3` 已安装并普通运行，原生备份/三模式通过；保护检查发现的 OneDrive Run 与冻结 profile 三项副作用已经定向恢复并由独立系统视图确认。验收仍未完成，已安装 `.3` 维护器和旧安装入口继续暂停，等待 `.4` 修复包；不要卸载或重启。见[诊断与恢复证据](YIMECORE_LOCAL3_REGISTRY_PRESERVATION_2026-09-03.md)。**

当前：2026-09-03 07:32 原生普通主令牌启动探针已经通过，见[启动修复记录](YIMECORE_STANDARD_USER_LAUNCH_FIX_2026-09-03.md)。原 `.2` 候选仍封存，不再安装。新 `.3` 候选已纳入修复并构建通过，使用普通 Explorer 双击 `Install-YimeCore-Local-Dev.cmd` 的原生验收入口；[新候选记录与固定哈希](YIMECORE_LOCAL_PRODUCT_LOCAL3_2026-09-03.md)。新包仍未安装，不能把探针 PASS 当成安装验收 PASS。
