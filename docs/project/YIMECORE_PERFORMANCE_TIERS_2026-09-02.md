...
本文是 2026-09-02 的历史性能分析，以下“当前/正式”均指当时决策。2026-09-04 的活动目标和入口以 [平台恢复计划](YIMECORE_PLATFORM_RESUMPTION_2026-09-04.md) 为准；本文模拟数字不作为新目标的实机通过证据。
...
- 当前正式门槛只保留主流档和超前档，两档模拟的功能正确性、暂定交互预算和进程私有内存预算全部通过：主流档 E1/E2 分别为 2.08–3.86 ms 和 25.65–28.14 ms；超前档分别为 2.00–3.75 ms 和 8.53–12.94 ms。
...
合并评分读缓存后的代码进行了 5 轮历史复测、每轮 100 个交错样本、每样本 5000 次回放。其中两轮随当时的性能脚本生成（含一次提交后干净复跑），另三轮独立重复：
...
1. **P0：补活动档实体机与冷启动。** 覆盖 16 GB 主流 Windows 11/NVMe 和 32 GB 以上新平台；采集冷启动、首键、长句、连续输入、后台负载和 Word/浏览器/Excel 宿主数据。
...
- 第二轮历史对照证据（本文仅引用主流/超前数据）：`C:\dev\Yime\.tmp\yimecore-tier-performance\final-round2-clean-20260902\summary.json`，SHA-256 `93071b4f20d1650460519327e6422b4be135cab7c6a742ea1b39889e917d6e14`
...
- 当前配置：`C:\dev\Yime\tools\yimecore\performance-tiers.json`；本机默认 profile 与恢复目标的 experiment_profiles 分开，不引用历史配置哈希作为现值。
