...
2026-09-07 DP1-P 进行中：版本身份准入门禁已固定并以 PS5／PS7 合成真值表验证，要求 old 与 successor 都具备受支持版本、canonical installer path 和 strict receipt，successor 还须携带 durable/current evidence；版本必须按 SemVer 严格递增，Windows 路径按大小写不敏感语义判重。当前源码身份已从 `1.4.0-dev` 推进到 `1.4.0-dev.1`，预期叶为 `YIME-1.4.0-dev.1-setup.exe`，固定 PE 数值版仍为 `1.4.0.0`，正式目标仍为尚未发布的 `v1.4.0`。这一步只预留 distinct leaf；新的 exact-HEAD isolated candidate 尚未构建，actual canonical／旧 installer／receipt 未改，`actual_canonical_migration_admitted=false`，安装、签名、注册、产品进程、默认输入法和用户数据均未触碰。
