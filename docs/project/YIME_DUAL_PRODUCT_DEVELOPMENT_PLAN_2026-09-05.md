...
2026-09-08 当前健康候选增量：[YimeCore local.13 健康候选与公共归档](YIMECORE_LOCAL13_HEALTH_CANDIDATES_2026-09-08.md)将健康协议纳入来源 `e2d9d422` 的独立普通候选，并准备不同身份的故障派生包。普通候选在 PS5/PS7 的隔离健康检查中各通过两轮、每轮两个角色；各自有进程退出事实单列，Broker 退出码 1 不被改记为 0。候选的 818 项源码 ZIP 不代表随后修改的本轮工具来源；工具、整组 CI、新归档与固定输入绑定见该报告，输入租约定向回归两版各 59 项已通过。此增量属于 YimeCore 未安装候选的健康观察，未给旧 local.12 增加协议或在采集 v2 中强制探测，也未完成真实维护、引擎就绪或登录启动验收。L6/local/public、Rime/PIME DP1-U 四类实际门禁及 DP2/DP3 均不提升；下方 219 等来源数量和“候选未重构”描述保留为相应阶段。
