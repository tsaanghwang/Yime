...
| DP1 双产品基线与隔离夹具 | 从当前源码核对两版身份、进程、注册、目录、构包输入及卸载所有权；固定各自来源清单；区分独立发布与旧 E7 可选跨产品切换证据；补“另一版不存在”与跨产品写入拒绝的隔离合同测试 | [DP1-J 历史记录](YIME_DUAL_PRODUCT_DP1_J_ISOLATED_MEMBERSHIP_AND_SUPERSESSION_2026-09-06.md)固定了两套隔离协议；[DP1-K](YIME_DUAL_PRODUCT_DP1_K_NSIS_COMPILER_INTERVAL_2026-09-06.md)已接入 fresh compiler stage 全编译区间检测；[DP1-L](YIME_DUAL_PRODUCT_DP1_L_RETAINED_RECEIPT_PUBLICATION_2026-09-06.md)已接入严格 reader 的内容寻址 evidence 和 receipt-only supersession／恢复 API；[DP1-M](YIME_DUAL_PRODUCT_DP1_M_BUILD_EVIDENCE_ADMISSION_2026-09-06.md)已将 interval 固定为新 build／receipt admission 的强制语义；[DP1-N](YIME_DUAL_PRODUCT_DP1_N_INSTALLER_RECEIPT_TRANSACTION_2026-09-07.md)固定 `.tmp` 隔离的 installer＋receipt identity replacement 与恢复合同；[DP1-O](YIME_DUAL_PRODUCT_DP1_O_ISOLATED_CURRENT_CANDIDATE_2026-09-07.md)完成 exact-HEAD clone 内的 `1.4.0-dev` disabled candidate；[DP1-P](YIME_DUAL_PRODUCT_DP1_P_VERSIONED_SUCCESSOR_CANDIDATE_2026-09-07.md)固定严格递增身份门禁并构建 `1.4.0-dev.1` successor；[DP1-Q](YIME_DUAL_PRODUCT_DP1_Q_CANDIDATE_EVIDENCE_ARCHIVE_AND_ACTUAL_MIGRATION_REVIEW_2026-09-07.md)以 PS5／PS7 各 24/24 固定仓内 `.tmp` 归档夹具、各 23/23 固定纯数据迁移审查合同；[DP1-R](YIME_DUAL_PRODUCT_DP1_R_STATIC_TRUST_CLOSURE_2026-09-07.md)再由双 shell 各 15/15 和两次实际候选只读复核，关闭 full payload static、non-OS compiler inputs 与 unsigned／disabled static-scope generated-uninstaller trust；[DP1-S](YIME_DUAL_PRODUCT_DP1_S_OFF_REPOSITORY_ARCHIVE_2026-09-07.md)把现有候选证据发布到所有 Git worktree 之外并由 fresh PS5／PS7 在原目录不可用时复核，完成当前进程／静态归档范围。Explorer 独立启动、目录 metadata durability、hardware power loss、hostile same-SID physical prevention、完整 OS 工具链、签名、migration／adapter／DP1-N transaction 仍未执行或未准入，因此 DP1 未完成 |
...
近期实施顺序：YimeCore 当前源码已同步重构未安装的 local.13；DP1-D 至 DP1-P 已依次固定注册所有权、密封构包输入、隔离事务、连续成员监测、strict receipt、installer＋receipt replacement、current candidate 和 versioned successor 的源码／构建／夹具合同。DP1-Q 固定仓内 `.tmp` 候选证据归档夹具和纯数据 actual migration 审查合同；DP1-R 已以独立派生准入关闭现有候选的 full payload static、non-OS compiler inputs 和 unsigned／disabled static-scope generated-uninstaller trust；DP1-S 已完成当前进程／静态范围的真实仓外 capsule 及 source-unavailable 双 shell reopen，并保留 Explorer、目录／断电／same-SID 物理 non-claims。下一步按序执行 DP1-T 真实 adapter／一次授权／migration，DP1-U 注册／回滚／removal／Runtime；在对应阶段另行通过前，不得调用 DP1-N 改动 actual canonical，不得安装、签名或交付，也不得把 ARM64 交叉产物标作原生证据。之后才按候选包变更范围安排 registered/live host、必要重启和 legacy migration。取得用户 L5 最终确认后才完成 L6；DP1、DP2、DP3 均不能借当前 local.12、静态构包或合成协议提前记为完成。
