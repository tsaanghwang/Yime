...
| DP1 双产品基线与隔离夹具 | 从当前源码核对两版身份、进程、注册、目录、构包输入及卸载所有权；固定各自来源清单；区分独立发布与旧 E7 可选跨产品切换证据；补“另一版不存在”与跨产品写入拒绝的隔离合同测试 | [DP1-J 当前记录](YIME_DUAL_PRODUCT_DP1_J_ISOLATED_MEMBERSHIP_AND_SUPERSESSION_2026-09-06.md)：隔离夹具已取得连续 NSIS tree-membership 检测和内容寻址／原子 head／journal supersession 协议的 PS5／PS7 证据。[DP1-K](YIME_DUAL_PRODUCT_DP1_K_NSIS_COMPILER_INTERVAL_2026-09-06.md)已接入 fresh compiler stage 全编译区间检测，supersession 尚未使用严格 receipt-v2 reader、迁移耐久 evidence 或修改 canonical receipt；跨进程、断电／目录耐久、hardlink、并发替换及真实事务 adapter 均未通过，因此 DP1 未完成 |
...
近期实施顺序：YimeCore 当前源码已同步重构未安装的 local.13；DP1-D 固定注册所有权和纯内存事务要求，DP1-E 封存声明 PE 输入，DP1-F 建立全量复制 stage，DP1-G 让真实 NSIS 只消费该 stage并完成静态归档核对，DP1-H 又把 stage、include、禁用构建和 postbuild 写入 canonical v2 receipt，并固定 303 个已知 NSIS 输入。DP1-I 在完全隔离的文件系统夹具中固定了 journal、replay 判定、类型化合成快照和精确 removal 语义；DP1-J 首批又固定连续 tree-membership 检测和内容寻址／原子 head／journal supersession 协议。DP1-K 已完成 fresh compiler stage 与全 makensis 区间检测接线，并验证真实最小编译夹具；完整产品构包和 canonical receipt 未刷新。下一项把实际严格 receipt-v2 reader、耐久 evidence、跨进程 replay／lock 和目录耐久边界接入，再在 publication lock 内完成 canonical v2→v2 supersession。只有这些先决条件成立后才接入真实 adapter，并补子进程硬崩溃、注册收敛、逐维回滚、并发安全 removal 和非提升 Runtime readiness。在另行交接前，安装／卸载及 tagged-release 硬阻断不得解除。之后才按候选包变更范围安排 registered/live host、必要重启和 legacy migration。取得用户 L5 最终确认后才完成 L6；DP1、DP2、DP3 均不能借当前 local.12、静态构包或合成协议提前记为完成。

2026-09-06 当前增量：[DP1-J 隔离成员监测与 receipt supersession 协议首批](YIME_DUAL_PRODUCT_DP1_J_ISOLATED_MEMBERSHIP_AND_SUPERSESSION_2026-09-06.md)完成。membership monitor 在 PS5／PS7 各 15/15，能检测同进程和独立同 SID 进程的瞬时文件／目录／rename 变化并对 overflow、异常取消和根替换前提失败关闭；supersession fixture 各 16/16，覆盖内容寻址 generation、单一原子 head、CAS、密封 journal、未决事务互斥及限定半写恢复。两者均只操作 fresh `.tmp` 夹具。监测未包围真实 `makensis`，supersession 未使用实际严格 receipt-v2 reader、未迁移非耐久 evidence、未修改 DP1-H canonical receipt，且跨进程崩溃／replay、目录耐久、hardlink 和并发替换仍未证明。未触碰安装器、产品进程、真实注册表、用户数据、默认输入法、生产 Rime/PIME 或 local.12；五项真实待办及全部硬阻断保持。
