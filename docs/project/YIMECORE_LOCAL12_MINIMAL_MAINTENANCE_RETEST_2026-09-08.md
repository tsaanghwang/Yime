...
2026-09-08 当前健康适配器准备：[原生健康适配器与进程引用](YIMECORE_NATIVE_HEALTH_ADAPTER_2026-09-08.md)继续推进[已归档的 local.13 健康候选](YIMECORE_LOCAL13_HEALTH_CANDIDATES_2026-09-08.md)。新增 `References` 入口接收原始 Runtime/Broker `Process` 对象，固定受审源码编译为模块私有类型；`DuplicateHandle` 复制原句柄并持续检查原对象关联、创建时间、活性与父子关系，无 PID 重开替代，默认全局发现仍保持原严格条件。两版 PowerShell 各通过默认 89 项和显式自有 stub 100 项，候选 `8f7e44…` 经完整健康适配器的实际组合结果见新报告。候选、818 项源码 ZIP 及既有 188 文件公共归档未修改，本轮工具独立绑定源码哈希。采集 v2 的恢复目标仍为不含健康协议的 local.12，未强制加入探测；故障包仍只作准备介质，真实恢复、回退、卸载及登录启动未执行或未据此验收。L5 用户确认保持有效，L6/local/public 仍为 false；下方准备步骤与未实现项按对应历史阶段理解。
...
2026-09-08 终态候选阶段记录：[新终态候选及读取器](YIMECORE_LOCAL13_OUTCOME_CANDIDATES_2026-09-08.md)已完成普通包 `1fd547…`、故障包 `7a70ba…` 和当时的新仓外归档 `1d74e0…`，两包均携带 `9f69…` 控制器。读取器双 shell 各 224 项通过，固定输入租约在该阶段核验该归档。以下旧 `e65…` 归档缺少终态协议的叙述只适用于旧工件；该时点原生事务编排、数据保护尚待实现，真实维护验收尚未发生。身份、数量和待办不覆盖顶部新健康候选，本轮仍未执行针对 local.12 的维护命令。
...
以上是已记录的预期身份，未来维护窗口必须重新核对实际安装与发起 SID；本次没有读取当前用户状态来作新证明。新维护窗口时间、授权引用、运行 ID 和新鲜用户状态备份路径仍未确定；故障候选 manifest 按顶部所引各阶段报告分别绑定，不能沿用本文件初次准备时的未确定状态，也不能把公共工件归档当作新鲜用户状态备份。
