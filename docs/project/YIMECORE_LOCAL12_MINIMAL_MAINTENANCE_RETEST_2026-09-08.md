...
2026-09-08 当前健康候选准备：[local.13 健康候选与公共归档](YIMECORE_LOCAL13_HEALTH_CANDIDATES_2026-09-08.md)记录从源码 `e2d9d422` 构建的新普通包及独立故障派生包。普通候选的 PS5/PS7 隔离健康检查各通过两轮、每轮两个角色，检查脚本退出 0；三个自有进程的实际退出结果单列，Broker 退出码 1 保留。该候选的 818 项源码 ZIP 不包含随后变更的本轮工具，工具来源、新归档及固定输入绑定见报告；输入租约的私有类型和目录重命名保护定向回归两版各 59 项通过，整组 CI 结果另行记录。故障包仍属准备介质，健康响应不代表引擎就绪、配置采用、实际回退或重启登录。采集 v2 恢复目标仍是原 local.12，未对不支持协议的旧版本强制健康检查。本轮未执行针对 local.12 的实际维护；L5 用户确认及 L6/local/public 的 false 结论保留。下方候选待重构、健康协议待实现等属于历史阶段。
