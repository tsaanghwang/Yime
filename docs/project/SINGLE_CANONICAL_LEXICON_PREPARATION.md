...
变长规则当前为：合并相邻且完全相同的音元，并保留虚首音作为零声母音节的显式边界。省键规则在此基础上，还依据干音音质组和调级元数据省略同音质连续段的中调；不再额外删除虚首音。三种模式因此都可把首音位置用于连续输入时的自动分词。对应 schema 同时启用 `enable_sentence` 和 `sentence_over_completion`，避免词条补全候选阻止 Rime 生成整句候选。
