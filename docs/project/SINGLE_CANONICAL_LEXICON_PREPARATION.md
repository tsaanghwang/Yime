...
> 实施状态（2026-07-11）：离线单源生成器已经落地。项目只导入
...
本方案已经把 Yime 收敛为“一个外部真源、三套内部运行产物”：
...
阶段 0 的差异审计和阶段 1—3 的单源生成、导入与运行约束均已完成。
后续若核心等长真源发生变化，使用仓库现行
`tools/import-yime-core-lexicon.ps1` 入口校验证据清单和输入 SHA-256，
确定性生成三套运行词典，并按测试指南完成真实 Rime、安装包和安装态验证。
本节不再作为“下一次实现”计划；当年的设计取舍和阶段划分仅作为历史依据保留。
