# Yime 自研栈独立化与双产品并列开发试验

> 当前开发过程决定（2026-09-05，优先于下方早期替换路线）：见[双产品开发过程决定](YIME_DUAL_PRODUCT_DEVELOPMENT_PLAN_2026-09-05.md)。YimeCore 为功能开发主线；Rime/PIME 为独立可选、长期稳定维护的产品和可选行为对照，不要求功能完全同步。共存是两个输入法可同时安装、各自独立运行；双方也必须在另一方未安装时工作。原“迁移期回退后退役”不再是默认路线，任何默认切换、卸载或退役均须另行授权。历史结果、回退载荷和未完成门禁保持原有证据等级，本次方向批准不等于发布或安装验收通过。

> 当前实施路线：见[本机独立产品实施计划](YIMECORE_LOCAL_PRODUCT_IMPLEMENTATION_PLAN.md)。2026-09-02 22:21 的[原生数据与回退闭环](../YIMECORE_NATIVE_LOCAL_CLOSURE_ACCEPTANCE_2026-09-02.md)已经通过；下一阶段为自包含构包与安装维护产品化。下方较早的阶段状态和旧包摘要保留为历史，不再表示本机恢复/回退仍待完成。

> 当前范围决定（2026-09-04，优先于下方历史阶段记录）：主流 x86-64 与 ARM64 机型试验已恢复，与本开发机 x64/x86 WOW64、L5/L6 同步推进。当前安装包不因试验解冻而变为跨平台安装包；旧身份 payload 只读保留，新产物必须从当前源码和身份重建。详见[平台恢复计划](YIMECORE_PLATFORM_RESUMPTION_2026-09-04.md)。

> 状态（2026-09-02）：E0 至 E5、E6-A、E6-B1、E6-B2a、E6-B2b、E6-B3a、E6-B3b、E6-B4a、E6-B4b、E6-B4c readiness、E6-B4d、E6-B5、E6-B6、E6-B7、E6-B8、E6-C 与 E6-D 的功能和独立化准备门禁已有通过证据；人工升级重启后的活动包为 `45b389e5` / manifest `944e300e`，安装态 x64/x86 注册宿主通过。发现 Run 陈旧值并修复当前值，下一次重启仍需复核；新 Word 已确认加载该包 DLL，裸数字组字、`tf-cN`/Shift+1 提交与英文 Shift 按键通过。用户物理操作后，同一 Word PID 的 4 次左击、右键菜单打开及菜单命令成功事件已补齐，真实任务栏验证通过。详见[重启后核验记录](../YIMECORE_TRIAL_POST_REBOOT_2026-09-02.md)
> 切换状态：当前仍为未签名并列试验版；签名证书正在申请，等候审批，暂缓相关事项。历史主流/超前两档模拟已通过预算，E3 历史最大绝对 p95 增量 `1.50 µs`，尾部比值作为非阻塞告警。当前实测证据来自本机；本机 x86 构建、注册宿主及 Firefox/Notepad++ 真实宿主已通过。主流 x86-64 与 ARM64 试验已恢复，但目标实机验收仍待完成。本机 x64 L5 日常使用与 L6 封存继续推进，未批准切换生产组件。
...
试验不是一次性重写，也不以复刻 librime/PIME 内部结构为目标。现有 Rime/PIME 链保留为独立可选产品、稳定生产基线及可选回退/行为对照路径，不要求以 YimeCore 取代它。YimeCore 独立化门禁仍须单独通过；任何系统默认输入法改变必须另行批准，不因双产品开发方向成立而自动执行。
...
早期引擎对照结构（历史试验路径，不是当前双产品的运行依赖关系）：
...
当前开发目标是两套互不依赖的产品结构；完整独立安装和共存维护能力仍须逐项验收：

```text
Rime/PIME 产品                         YimeCore 产品
PIMETextService.dll                    YimeTextService.dll（薄 TSF/COM 层）
    -> PIMELauncher / Go server.exe        -> IPC / YimeBroker.exe
    -> nativeBackend / librime            -> CandidateEngine / YimeCore
    -> 本产品词典与用户数据                -> 只读静态索引与本产品用户数据
```

各自拥有安装、运行、升级、卸载、恢复和可写状态；不得共享活动学习库、设置、自启动或服务依赖。用户可选择仅装其中一个或同时安装两个；对方缺失不得阻止本产品工作。RimeAdapter 可继续用于 Rime 产品和隔离对照工具，但不进入 YimeCore 运行包。可选对照帮助解释差异，不把 Rime 的行为视为必须照抄的标准，也不替代 YimeCore 独立运行证据。
...
E3-C 受控纠错与可选线性重排当前结果：

- 用户在句中改段并最终显式提交时，v3 用户模型把被替代首句的有界负向计数、最终句及分段的正向计数作为同一 mutation 持久化；临时改选、`Clear` 和 `Reset` 不学习。正向确认会逐级抵消旧负向计数，遗忘可删除只有负向证据的候选及上下文。
- 可选稀疏线性感知机默认关闭，只对 beam 已保留且词优先结构相同的路径增加独立 `Reranker` 分。每个特征权重和特征值均有硬上限；在线更新、journal 恢复、v3 快照、备份恢复和 mutation ID 幂等共用既有用户模型边界。
- v1/v2 读取继续受支持；正常保存升级到 v3。显式导出 v1 回滚快照会保留旧版可表达的正向选择、上下文和幂等账本，并有意丢弃 v3 负反馈与重排权重。
- BCC 每日报告只运行无用户模型实例，输出 graph 缺边、beam 淘汰、最终排序落后和无效输入诊断，以及按模式分级的开发者告警。日报预期文本不进入训练；它是改进提示，不是自训练数据源。

E3-C 尚未批准生产默认启用线性重排器。启用决策必须使用独立审阅训练集和固定 holdout，证明三模式 Top-1、可纠正率、延迟和回滚均不退化；随机日报不能同时承担训练和验收。
...
- `CandidatePopup` 是无图标、无皮肤的 Win32 `WS_EX_NOACTIVATE` tool window，只使用系统颜色。窗口最多显示九项，文本继续来自 `CandidateListUIElement` 的 `⇧1` 至 `⇧9` 标签，不产生裸数字候选序号；常规字号为 12 points，小/大两档为 10/16 points。
...
- 包内增加独立输入法工具栏、现有受审音节/反查数据和 `YinYuan-Regular.ttf`。字体通过文本服务进程私有加载，不修改系统字体注册；显示编码为音元拼音时强制使用该字体，普通候选字体选择不能覆盖 PUA 字符渲染。这些数据原本就在当前仓库中，没有从原型仓库复制数据。
- 安装态多索引 Broker 以 `variable`（界面名“变长”）为默认模式，并用同一进程验证默认、变长、等长和省键会话。每种会话都须返回“你好”的精确词典项以及键位、音元和带调拼音三种注释。
...
- Word 并列试装又暴露出普通 `Space`/`Enter` 未接入候选提交。表层现将这两个无 Shift 键映射为当前第一候选；只有候选存在时才吞键，`Shift+Space`/`Shift+Enter` 继续交给宿主。契约、Broker 桥、直接 TSF 和注册宿主回归均须覆盖这条键路，且不改变裸数字与 `Shift+1` 至 `Shift+9` 的既有约束。
- 并列试装同时暴露了空候选和退格同步缺口：错误编码会让自有候选窗直接消失，退格只修改宿主表面文本而未修改 Broker raw input，下一键因而会使已删编码“复活”。表层现将 `Backspace` 发送为内核退格事件；raw input 非空但无命中时保留 composition 与候选窗，显示“无匹配候选，按退格修改”。回归轨迹固定为“有效编码 -> 追加错码 -> 空候选仍可编辑 -> 退格恢复 -> 再退格并继续输入”，严禁已删字符回流。
- 候选导航键同步接入：`PageUp`/`PageDown` 与 `←`/`→` 分别向 Broker 请求上一页/下一页，`↑`/`↓` 在当前页移动高亮，自有候选窗与 `ITfCandidateListUIElement::GetSelection` 必须同步反映高亮。`Enter`/`Space` 提交当前高亮项，`Shift+1` 至 `Shift+9` 仍绕过高亮按固定 ordinal 直选。组合串非空时这些导航键不得泄漏给宿主滚页或移动文档光标。
...
E6-C 在不切换生产输入法的前提下增加搜索排列、显示设置和 Windows 原生语言栏控制：

- 搜索树的短前缀不再把九项缓存误当成完整结果集；分页请求进入完整前缀范围，连续页面保持稳定排序且不重复。排序池明确分层为“词典精确项、动态组句、前缀补全”，因此完整词条不会再被单字权重相加产生的组句挤出第一页。
- E6-C 多索引试验版先采用不分层的全量常驻基线：Broker 启动时一次读入 full、variable、shorthand 三份完整系统索引，事务切换的新索引也保持全量常驻；旧会话仍租用原代索引。包实验记录启动耗时、Broker 内存、三模式各 250 轮完整输入链的 P50/P95/P99/最大延迟和前后半程漂移，并把“全部模式常驻、重启后仍常驻、无严重迟延或粘滞”列为安装前门禁。该策略只用于 YimeCore 自研栈试验版，不改变生产 Rime/PIME。
- 连续输入进入第二个词语时，搜索格把已经完成的左侧路径与仍是合法系统词库前缀的未完成尾段连接起来；因此中间节点即使没有新的整词精确项，也继承显示左侧候选，而真正无效的编码分支仍返回“无匹配候选”。组句排序先比较系统词库分段数，再比较同分段数内的静态权重，避免多个高权重单字之和把更完整的两段词语路径压到末位。E6-C 包门禁逐键验证 full、variable、shorthand 的“你好”+“排序”全部中间节点非空，并要求完整组句“你好排序”为首选。
- 显式提交的动态整句沿用独立用户模型中的完整 `(code,text)` 记录，重开会话和 Broker 后可直接召回，不写 Rime userdb、系统词典或另一份可变词库。召回时先用当前索引的精确词段按目标文本约束重建可编辑路径；普通 beam 已淘汰的历史校正句只要仍能被当前索引完整证明，就恢复原句各段并在改选一段时锁住其它段，无法证明的记录只作为无结构整句候选。`Ctrl+Left`/`Ctrl+Right` 在分段句内移动活动段，`Ctrl+Delete` 遗忘当前高亮候选并原地刷新候选而不提交或清空 composition；其它 Ctrl/Alt 组合继续交给宿主，裸数字和 `Shift+1` 至 `Shift+9` 契约不变。E6-B2a 门禁为每种模式启用独立临时快照和哈希链日志，在 x64/x86 以及 Broker 重启后重复验证学习、遗忘、相邻段导航和 25 轮首/中/尾纠错。
