...
> 当前范围决定（2026-09-02，优先于下方历史阶段记录）：只推进本开发机原生 x64。ARM64、x86、老旧 x64、其他实体机和硬件分档模拟全部冻结，不作为当前阻塞。先完成本机独立核心，再评审恢复其他目标。默认构建/升级/性能/E7 入口已按此收敛；冻结不等于通过，也不授权生产切换。详见[本开发机优先阶段](YIMECORE_DEVELOPMENT_HOST_SCOPE.md)。
...
> 切换状态：当前仍为未签名并列试验版；签名证书正在申请，等候审批，暂缓相关事项。历史主流/超前两档模拟已通过预算，E3 历史最大绝对 p95 增量 `1.50 µs`，尾部比值作为非阻塞告警。今后仅本机原生 x64 测量；ARM64/x86 和其他硬件验收延期，不再阻塞本机开发。本机更广 x64 宿主、回退与独立发布维护闭环仍须完成，未批准切换生产组件。
