...
> 当前范围决定（2026-09-04，优先于下方历史阶段记录）：只使用本开发机；原生 x64 Runtime/Broker、x64 TSF 与 WOW64 32 位应用中的 x86 TSF 为活动范围。ARM64、老旧 x64、其他实体机和硬件分档模拟仍冻结。x86 表面必须从当前源码和身份重建，旧 x86 payload 只读保留。详见[本开发机优先阶段](YIMECORE_DEVELOPMENT_HOST_SCOPE.md)。
...
> 切换状态：当前仍为未签名并列试验版；签名证书正在申请，等候审批，暂缓相关事项。历史主流/超前两档模拟已通过预算，E3 历史最大绝对 p95 增量 `1.50 µs`，尾部比值作为非阻塞告警。当前只在本机测量；本机 x86 构建、注册宿主及 Firefox/Notepad++ 真实宿主已通过，ARM64 和其他硬件验收仍冻结。本机 x64 L5 日常使用与 L6 封存仍须完成，未批准切换生产组件。
...
- [首次独立发布保留与回退方案](YIMECORE_INDEPENDENT_RELEASE_RETENTION_PLAN.md) 已起草，尚待批准和实际演练；不因文档存在而将外部验收字段置为 true。
