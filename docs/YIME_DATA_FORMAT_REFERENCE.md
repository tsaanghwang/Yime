...
派生时会合并相邻同码，但保留零声母音节的虚首音，作为连续输入时的显式音节边界；
省键模式只在此基础上省略符合规则的中调，不再删除虚首音。

**规模**：当前 1729 条数据 + 1 行标题，约 19 KB。实际数量应与 handoff 清单的
...
**规模**：当前审计库存 1732 条，约 33 KB。运行时有布局编码的音节为 1729 条；
`guai2`、`ra4`、`tin4` 是经 handoff 清单声明的源端专用音节。
