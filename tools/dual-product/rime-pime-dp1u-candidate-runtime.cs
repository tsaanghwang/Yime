// Native process/file identity and bounded named-pipe transport primitives.
// The PowerShell candidate controller owns admission, launch and directed stop.
using System;
...
namespace Yime.CandidateRuntime {
...
namespace Yime.CandidateRuntime {
    public static class PipeFacts {
        ...
        public static int ServerPid(System.IO.Pipes.NamedPipeClientStream pipe) {
            uint pid;
            if (!GetNamedPipeServerProcessId(pipe.SafePipeHandle, out pid) || pid == 0)
                throw new System.ComponentModel.Win32Exception(System.Runtime.InteropServices.Marshal.GetLastWin32Error());
            return checked((int)pid);
        }
        public static string ReadLine(System.IO.Pipes.NamedPipeClientStream pipe, int timeoutMs) {
            var bytes = new System.Collections.Generic.List<byte>(); var one = new byte[1];
            var clock = System.Diagnostics.Stopwatch.StartNew();
            while (bytes.Count <= 32768) {
                int left = timeoutMs - checked((int)clock.ElapsedMilliseconds);
                if (left <= 0) throw new System.TimeoutException("Runtime response timed out.");
                var read = pipe.ReadAsync(one, 0, 1);
                if (!read.Wait(left)) throw new System.TimeoutException("Runtime response timed out.");
                if (read.Result == 0) throw new System.IO.EndOfStreamException();
                if (one[0] == 10) return new System.Text.UTF8Encoding(false,true).GetString(bytes.ToArray()).TrimEnd('\r');
                bytes.Add(one[0]);
            }
            throw new System.IO.InvalidDataException("Oversized runtime response.");
        }
        public static void WriteLine(System.IO.Pipes.NamedPipeClientStream pipe, string line) {
            byte[] data = new System.Text.UTF8Encoding(false,true).GetBytes(line + "\n");
            if (data.Length > 32768) throw new System.IO.InvalidDataException("Oversized runtime request.");
            if (!pipe.WriteAsync(data, 0, data.Length).Wait(5000)) throw new System.TimeoutException("Runtime request timed out.");
        }
    }
}
