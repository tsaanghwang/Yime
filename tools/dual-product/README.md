...
- `rime-pime-package-receipt-v2.ps1` 与显式 finalizer 把一个候选的 v1 predecessor、stage、生成 include、build-result、固定 toolchain 和 postbuild-result 绑定成 canonical v2。DP1-M 后，新 preparation 只接受精确的 tagged membership-interval build schema；旧 build-result-v2 只保留严格读取兼容且不得夹带新区间字段，未知／裸数字未来版本均拒绝。旧 canonical 仍明确 `evidence_artifacts_durable=false`；发布 v2 后旧 v1 builder／签名流程拒绝降级。
- `rime-pime-receipt-v2-store.ps1` 把严格 reader 所需证据按 SHA-256 保留，并提供共享 publication lock 下显式的 v2→v2 receipt-only supersession 与 persistent-intent recovery。只有同一历史收据的 retention-only 转换可继续使用旧 build-result-v2；任何改变过的 successor 在 Publish 与 Resume 两处都必须绑定当前 membership-interval 证据。该源码接线未自动迁移 actual canonical；硬件断电／目录元数据耐久、installer identity 替换和真实事务 adapter 仍待验收。
...
DP1-K 已把 fresh compiler stage 和真实最小 `makensis` 全区间监测接入 builder；DP1-L 又接入严格 reader 的内容寻址证据保留及 receipt-only v2→v2 supersession／恢复 API；DP1-M 进一步把该 interval 固定为新 build evidence／receipt preparation 的强制语义，并让旧代码、未知 schema、改过的 legacy successor 失败关闭。当前 contract source path 仍为 125 个、hash source 为 132 个，Python 合同 52/52；actual canonical 未迁移，完整产品构包未刷新。断电／目录耐久、恶意同 SID 并发替换、installer identity 原子替换和真实 transaction adapter 均待办。安装／卸载、tagged release、installed／registered／live-host 硬门不变，DP1、DP2、DP3、L5、L6 均未完成。
