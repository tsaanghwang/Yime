...
$candidate='C:\dev\Yime\.tmp\yimecore-local-product\local6-tip-preservation-rc1\package'
...
if(-not (Test-Path -LiteralPath $manifestPath -PathType Leaf) -or (Get-FileHash -LiteralPath $manifestPath).Hash -ine $expectedManifest){throw 'Pinned local.6 candidate is missing or changed.'}
...
if(-not $Execute){
    [ordered]@{action='local6-uninstall-preserve-data-and-reinstall';candidate=$candidate;manifest_sha256=$expectedManifest;
        fresh_backup_required=$true;uninstall_gap_verified=$true;reinstall_same_package=$true;
...
if($identity.User.Value -cne $expectedSid){throw 'Use the Windows account that owns the local.6 installation.'}
...
if((Get-FileHash -LiteralPath (Join-Path $oldRoot 'package-manifest.json')).Hash -ine $expectedManifest){throw 'Current installation is not the pinned local.6 package.'}
$oldContext=Assert-LocalProductInstalledContext $oldRoot $state
$oldLive=Assert-LocalProductLiveRuntime $oldContext
$evidence=Join-Path $env:USERPROFILE ('YimeCore Recovery Archives\local6-uninstall-reinstall-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'-'+[guid]::NewGuid().ToString('N').Substring(0,8))
...
try{
    ...
    if($LASTEXITCODE -ne 0){throw 'Pinned local.6 candidate audit failed.'}
    ...
    if($uninstallExit -ne 0){throw "Local.6 uninstall failed with exit $uninstallExit."}
    $uninstalled=$true
    ...
    if($midRegistry.native_com.exists -or
        ...
        @($midRegistry.trial_run).Count -ne 0 -or $midRegistry.uninstall.exists){throw 'Active local.6 registration was not completely removed.'}
    ...
    if($installExit -ne 0){throw "Local.6 reinstall failed with exit $installExit; preserve evidence and recovery archive."}
    $reinstalled=$true
    ...
    Write-Host "PASS: local.6 uninstalled with data preserved and reinstalled from the pinned complete package. Evidence: $evidence"
}catch{
    ...
    try{
        Write-Evidence ([ordered]@{schema_version='yimecore-local6-uninstall-reinstall-acceptance-v1';passed=$passed;stage=$stage;
            ...
            frozen_targets_executed=$false;reboot_requested=$false;failure=$failure;local_product_ready=$false;public_release_ready=$false}) (Join-Path $evidence 'summary.json')
