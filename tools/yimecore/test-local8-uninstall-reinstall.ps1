...
$path=Join-Path $PSScriptRoot 'invoke-local8-uninstall-reinstall.ps1'
...
Check ($text.Contains("`$candidate='C:\dev\Yime\.tmp\yimecore-local-product\local8-build-20260903-1705\package'")) 'candidate path is pinned'
Check ($text.Contains("`$expectedManifest='0354fd33fcae9171004ecd7c9a33f2e56bcf27c2cc99e58fbf857bc67e8e1fc2'")) 'local.8 package identity is pinned'
...
$out=Join-Path (Join-Path $PSScriptRoot '..\..\.tmp\yimecore-local-product') ('local8-uninstall-reinstall-contract-'+[guid]::NewGuid().ToString('N'))
...
Write-Output "PASS: $($checks.Count) local.8 uninstall/reinstall contracts. Evidence: $out"
