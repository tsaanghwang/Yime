...
if(-not (Test-Path -LiteralPath $manifestPath -PathType Leaf) -or (Get-FileHash -LiteralPath $manifestPath).Hash -ine $expectedManifest){throw 'Pinned local.8 candidate is missing or changed.'}
...
if(-not $Execute){
    [ordered]@{action='local8-uninstall-preserve-data-and-reinstall';candidate=$candidate;manifest_sha256=$expectedManifest;
...
if($identity.User.Value -cne $expectedSid){throw 'Use the Windows account that owns the local.8 installation.'}
...
if((Get-FileHash -LiteralPath (Join-Path $oldRoot 'package-manifest.json')).Hash -ine $expectedManifest){throw 'Current installation is not the pinned local.8 package.'}
$oldContext=Assert-LocalProductInstalledContext $oldRoot $state
$oldLive=Assert-LocalProductLiveRuntime $oldContext
$evidence=Join-Path $env:USERPROFILE ('YimeCore Recovery Archives\local8-uninstall-reinstall-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'-'+[guid]::NewGuid().ToString('N').Substring(0,8))
...
try{
    ...
    if($uninstallExit -ne 0){throw "Local.8 uninstall failed with exit $uninstallExit."}
    $uninstalled=$true
    ...
    if($midRegistry.native_com.exists -or
        ...
        @($midRegistry.trial_run).Count -ne 0 -or $midRegistry.uninstall.exists){throw 'Active local.8 registration was not completely removed.'}
    ...
    Write-Host "PASS: local.8 uninstalled with data preserved and reinstalled from the pinned complete package. Evidence: $evidence"
}catch{
    ...
    try{
        Write-Evidence ([ordered]@{schema_version='yimecore-local8-uninstall-reinstall-acceptance-v1';passed=$passed;stage=$stage;
