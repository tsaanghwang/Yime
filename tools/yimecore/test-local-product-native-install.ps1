...
$before=[ordered]@{protected=[ordered]@{production='unchanged';frozen='unchanged';user_tip='unchanged';default_override='original'}}
...
foreach($case in @('production','frozen','user_tip','default_override','com_path','run_path','run_kind','missing_run','duplicate_run','uninstall_sid','uninstall_state','uninstall_mode','install_location','display_name')) {
...
Check ($text.Contains('Assert-YimeCoreArchiveRecords $previous $saved.package_files')) 'old frozen-reference package is checked byte-for-byte after install'
