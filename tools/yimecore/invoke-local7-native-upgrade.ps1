...
$package='C:\dev\Yime\.tmp\yimecore-local-product\local7-build-20260903-1645\package'
$expectedManifest='0346bbe83eb3dab721e3bd75b14031a604dbdb7fbed041f9021834b7822690bb'
$expectedPreviousManifest='42e28f7de646d476c64e3ef441a4e60b17acb108f623949e990e8c23d05e2087'
$expectedManager='4680a94aada563e2fc988529dca27050c5ce43dd7e447577093e2a4a93d721c5'
...
if((Get-FileHash -LiteralPath $manifestPath).Hash -ine $expectedManifest){throw 'local.7 candidate manifest changed; refuse upgrade.'}
...
foreach($name in @('development-scope.ps1','local-maintenance-safety.ps1','local-package-contract.ps1','local-product-runtime.ps1')){
    ...
    if($record.Count -ne 1 -or (Get-FileHash -LiteralPath $path).Hash -ine $record[0].sha256){throw "Unverified local.7 helper: $name"}
    . $path
}
$manager=Join-Path $package 'maintenance\Manage-YimeCoreTrial.ps1'
if((Get-FileHash -LiteralPath $manager).Hash -ine $expectedManager){throw 'local.7 does not contain the reviewed TIP-preservation manager.'}
$managerText=Get-Content -LiteralPath $manager -Raw -Encoding UTF8
if(-not $managerText.Contains('Restore-FrozenUserTipSnapshot $migrationLegacyUserTipSnapshot') -or
    -not $managerText.Contains('Enable-TargetUserTip $userTipKey $profile')){throw 'local.7 TIP preservation or first-install enablement is missing.'}
...
if($LASTEXITCODE -ne 0){throw "local.7 Plan failed: $planText"}
$plan=$planText|ConvertFrom-Json
if([string]$plan.install_root -cne 'C:\Program Files\YimeCore Experimental Trial\yimecore-e6c-efb172e72ec7-0346bbe8'){throw 'Unexpected local.7 target root.'}
if(-not $Execute){
    [ordered]@{action='local7-native-upgrade-plan';candidate_manifest_sha256=$expectedManifest;expected_previous_manifest_sha256=$expectedPreviousManifest;
        target_sid=$expectedSid;plan=$plan;writes_requested=$false;repeats_identity_migration=$false;reboot_requested=$false;
        next_action='normal Explorer double-click Upgrade-YimeCore-Local7.cmd'}|ConvertTo-Json -Depth 10
    exit 0
}
...
if((Get-FileHash -LiteralPath (Join-Path $previous 'package-manifest.json')).Hash -ine $expectedPreviousManifest){throw 'Current install is no longer the repaired local.6 baseline; stop and re-plan.'}
if(Test-Path -LiteralPath ([string]$plan.install_root)){throw 'Planned local.7 target is occupied; preserve it and re-plan.'}
...
if($otherProcesses.Count){throw 'Close input-method tools from local.6 before the native upgrade.'}
$liveBefore=Get-YimeCoreLiveRuntimeEvidence $state
if(-not $liveBefore.passed){throw 'Current local.6 Runtime/Broker identity is not live and verified.'}

# Reuse only the read-only system-view snapshot helpers from the closed identity
...
function Read-ActiveUserTip {
    Read-YimeCoreSystemKey ([uint32]2147483651) "$expectedSid\Software\Microsoft\CTF\TIP\{E40FA752-BB96-461D-A51D-F40EB437EC65}"
}
function Assert-ActiveUserTipEnabled($Snapshot){
    if($null -eq $Snapshot -or -not $Snapshot.exists){throw 'Active per-user local product TIP is missing.'}
    $profile=$Snapshot.children.LanguageProfile.children.'0x00000804'.children.'{126F54C6-E9B1-4E22-8652-03224CBD49F9}'
    if($null -eq $profile -or -not $profile.exists){throw 'Active per-user local product language profile is missing.'}
    Require-CutoverValue $profile.values 'Enable' 4 '1'
}

$archive=Join-Path $env:USERPROFILE ('YimeCore Recovery Archives\local7-native-upgrade-'+(Get-Date -Format 'yyyyMMdd-HHmmss')+'-'+[guid]::NewGuid().ToString('N').Substring(0,8))
...
try{
    ...
    $activeUserTipBefore=Read-ActiveUserTip
    Assert-ActiveUserTipEnabled $activeUserTipBefore
    Write-AcceptanceJson $activeUserTipBefore (Join-Path $archive 'active-user-tip-before.json')
    ...
    if($LASTEXITCODE -ne 0){throw 'Current local.6 package integrity audit failed.'}
    ...
    if($LASTEXITCODE -ne 0){throw 'local.7 candidate integrity audit failed.'}
    $stage='fresh-backup'
    ...
    if($LASTEXITCODE -ne 0){throw "local.7 package upgrade failed with exit code $LASTEXITCODE; its transaction owns rollback."}
    $upgradeSucceeded=$true
    $stage='verify-installed'
    $context=Assert-LocalProductInstalledContext ([string]$plan.install_root) $state
    if($context.package.manifest_sha256 -ine $expectedManifest){throw 'Installed package is not the reviewed local.7 candidate.'}
    ...
    if($LASTEXITCODE -ne 0){throw 'Installed local.7 three-mode verification failed.'}
    ...
    $activeUserTipAfter=Read-ActiveUserTip
    Assert-ActiveUserTipEnabled $activeUserTipAfter
    Write-AcceptanceJson $activeUserTipAfter (Join-Path $archive 'active-user-tip-after.json')
    if(-not (Same $activeUserTipBefore $activeUserTipAfter)){throw 'Active per-user TIP changed during same-identity upgrade.'}
    ...
    $passed=$true;$stage='local7-native-upgrade-accepted'
}catch{
    ...
    try{
        Write-AcceptanceJson ([ordered]@{schema_version='yimecore-local7-native-upgrade-v1';passed=$passed;stage=$stage;failure=$failure;
            ...
            user_data_preserved=$passed;protected_registry_preserved=$passed;ordinary_runtime_verified=$passed;active_user_tip_enabled_and_preserved=$passed;reboot_requested=$false;
...
Write-Host "PASS: local.7 native upgrade, ordinary runtime, data and protected registration verified. Evidence: $archive"
