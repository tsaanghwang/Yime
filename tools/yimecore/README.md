...
2026-09-15 移除旧产品身份的 `run-e6b*.ps1` 阶段编排。它们不被当前构建或 CI 调用，历史源码可在 `80888e50` 查询；原始阶段报告和回滚载荷仍保留。C++ 契约、Broker 桥、composition 和注册宿主测试均保留在 `YimeTextServiceExperiment/tests/`。当前身份的构建及平台入口以上方列表为准，源码检查与已安装宿主验收分别记录。
