...
$outputDir = Join-Path $temporaryRoot 'particle-a-stage6a-audit'
$runnerDir = Join-Path $temporaryRoot 'particle-a-stage6a-runner'
$runnerPath = Join-Path $runnerDir 'main.go'
$goCache = Join-Path $temporaryRoot 'go-particle-a-stage6a-cache'
...
func main() {
	...
	flag.Parse()
	result, err := connectedspeech.RunParticleAStage6AAudit(connectedspeech.DefaultParticleAStage6AConfig(*repo))
	if err != nil {
...
if ($exitCode -ne 0) { throw "Particle-a Stage 6A audit failed with exit code $exitCode" }
