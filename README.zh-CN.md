...
音元输入法将拼音音节映射到结构化的键盘编码，首音遵循易记的规律（zh/ch/sh → 7/8/9，j/q/x → 3/2/1，z/c/s → 6/5/4）。提供三种编码模式：变长模式（默认）、等长模式（每音节固定4键，无重码）、省键模式（最短编码）。
...
在音元拼音中，首音分为实首音和虚首音两类。从音值关系看，实首音对应传统的非零声母，虚首音
对应传统的零声母；但首音的编码方式与流行拼音输入法不同，具体编码见下表。按本项目采用的汉语
语音学口径，零声母在《汉语拼音方案》中的书写表现包括隔音符号 `'` 以及 `y`、`w`，三者在音元
拼音中都由虚首音承载。虚首音还在连续输入中承担显式音节边界。
