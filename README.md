...
Current [DP1-M work](docs/project/YIME_DUAL_PRODUCT_DP1_M_BUILD_EVIDENCE_ADMISSION_2026-09-06.md) makes the completed DP1-K compiler-membership interval mandatory for newly prepared build evidence. The tagged schema is intentionally rejected by pre-DP1-M validators; current readers accept only that exact schema or read-only historical build-result-v2, and both publication and recovery reject a changed legacy successor. This is isolated schema/contract work, not a full product rebuild or canonical migration. The existing canonical receipt, disabled installer identity, installed products, default input method, and user data remain unchanged.

Current [DP1-N work](docs/project/YIME_DUAL_PRODUCT_DP1_N_INSTALLER_RECEIPT_TRANSACTION_2026-09-07.md) defines a fixture-only installer-plus-receipt identity replacement transaction. It durably stages the exact new versioned installer before publishing intent, rolls forward after intent through four recognized public states/transients, preserves the old installer, and uses no-replace write-through moves for stage, physical installer, intent, and completion ownership. The successor must be a durable strict current membership-interval receipt. This does not migrate the actual canonical receipt or installer, refresh a complete package, run or install either product, connect a real transaction adapter, or prove power-loss, directory-metadata, or hostile same-SID safety. The two products remain independently installable and runnable whether one or both are present.

Historical [DP1-O evidence](docs/project/YIME_DUAL_PRODUCT_DP1_O_ISOLATED_CURRENT_CANDIDATE_2026-09-07.md) rebuilt one unsigned, disabled x86/x64 candidate from exact HEAD `ba0e2dc2d42fee66ac4e707800c43c2630794f7a` in a detached clone, statically checked 181 outer and 11 nested-uninstaller entries, and produced a clone-local durable strict receipt-v2. The sealed candidate was `YIME-1.4.0-dev-setup.exe`, 41,431,585 bytes, SHA-256 `c55905a0c0cf518fb4eeda10f69d95c6d357b036461560a051accc67766699e2`; the final runner result SHA-256 was `d922c851ef74895a65424248f176d76d30cdd6cccdd5cd1385f88afdb961af0a`. The runner's protected actual-checkout snapshot did not change. At DP1-O, because the actual and isolated receipts used the same version and installer leaf, the DP1-N distinct-leaf prerequisite was false and actual canonical migration was not admitted. The clone-local process-interruption store was not outer `.tmp` retention or archival evidence, and no installer, adapter, signing, installed/live or hardware/directory-durability gate passed.

Current [DP1-P evidence](docs/project/YIME_DUAL_PRODUCT_DP1_P_VERSIONED_SUCCESSOR_CANDIDATE_2026-09-07.md) adds a strict SemVer and Windows-path identity gate, advances the reviewed source identity to `1.4.0-dev.1`, and rebuilds an unsigned, disabled x86/x64 successor from exact HEAD `9f536635f29960365afac32a2561bfaa62f84d1b`. The isolated candidate is `YIME-1.4.0-dev.1-setup.exe`, 41,455,231 bytes, SHA-256 `ac97f65af80e9686525a3c2f289670aaa14edb60a6effc80ce7cc3580333970d`; the pure-data gate reports `identity_transition_admitted=true`, while the protected actual checkout remained unchanged within the run. This admits only the isolated successor identity/evidence precondition for a separate migration review: `actual_canonical_migration_admitted=false`, and installer execution, signing, delivery, installed/live validation, outer evidence archival and ARM64 native validation remain false or unproven.

Current [DP1-Q evidence](docs/project/YIME_DUAL_PRODUCT_DP1_Q_CANDIDATE_EVIDENCE_ARCHIVE_AND_ACTUAL_MIGRATION_REVIEW_2026-09-07.md) adds a fixture-only candidate-evidence archive contract (PowerShell 5.1/7: 24/24 each) and a pure-data actual-migration review contract (23/23 each). The archive fixture remains inside this repository's `.tmp`, so DP1-P evidence has not been archived outside the repository; synthetic `review_ready=true` is neither an actual review nor execution authorization. Actual archive, review, migration, adapter use, the DP1-N actual transaction, installation, signing, delivery, and ARM64-native validation remain unexecuted or unadmitted. The next sequence is DP1-R full payload/non-OS/NSIS/generated-uninstaller trust, DP1-S real off-repository archival plus directory/power-loss/same-SID evidence, DP1-T real adapter/authorization/migration, then DP1-U registration/rollback/removal/runtime.

Current [DP1-R evidence](docs/project/YIME_DUAL_PRODUCT_DP1_R_STATIC_TRUST_CLOSURE_2026-09-07.md) derives a separate bounded admission from the existing DP1-P candidate, the complete compiler-interval record, independent PS5/PS7 postbuild extraction and an audit of the actual NSIS source bytes. It closes full-payload static and non-OS compiler-input scope and trusts the generated uninstaller only for this unsigned, disabled static-review scope. Historical false fields remain unchanged; physical same-SID prevention, the full OS toolchain, signature trust, signing, delivery, execution, installed hosts and canonical migration remain false. DP1-S off-repository evidence archival is next.

Current [DP1-S evidence](docs/project/YIME_DUAL_PRODUCT_DP1_S_OFF_REPOSITORY_ARCHIVE_2026-09-07.md) publishes the actual DP1-P evidence as an immutable 43-object capsule outside every Git worktree and repository `.tmp`. With the original candidate directory unavailable, fresh PS5 and PS7 child processes reconstructed the exact receipt-bound projection solely from the capsule, passed the strict reader, and verified the installer bytes; an idempotent rerun reused both verification records without overwrite. The children had no package identity but retained a packaged Codex ancestor, so Explorer-launched independence remains false. Directory-metadata durability, hardware power loss, hostile same-SID physical prevention and canonical migration also remain false. DP1-T actual adapter and exact authorization-bound migration are next.
...
| [DP1-N Installer and Receipt Transaction](docs/project/YIME_DUAL_PRODUCT_DP1_N_INSTALLER_RECEIPT_TRANSACTION_2026-09-07.md) | Fixture-only durable installer-plus-receipt identity replacement and fail-closed recovery contract |
| [DP1-O Isolated Current Candidate](docs/project/YIME_DUAL_PRODUCT_DP1_O_ISOLATED_CURRENT_CANDIDATE_2026-09-07.md) | Exact-HEAD disabled candidate, static archive evidence, scoped durable receipt and its then-current same-leaf migration blocker |
| [DP1-P Versioned Successor Candidate](docs/project/YIME_DUAL_PRODUCT_DP1_P_VERSIONED_SUCCESSOR_CANDIDATE_2026-09-07.md) | Strict version/leaf admission and an exact-HEAD `1.4.0-dev.1` isolated candidate without actual canonical migration |
| [DP1-Q Candidate Evidence Archive and Actual Migration Review](docs/project/YIME_DUAL_PRODUCT_DP1_Q_CANDIDATE_EVIDENCE_ARCHIVE_AND_ACTUAL_MIGRATION_REVIEW_2026-09-07.md) | Fixture-only archive and pure-data review contracts; no real off-repository archive, authorization, adapter, or migration |
| [DP1-R Static Trust Closure](docs/project/YIME_DUAL_PRODUCT_DP1_R_STATIC_TRUST_CLOSURE_2026-09-07.md) | Actual-candidate static payload, non-OS input and generated-uninstaller trust admission with historical and higher-level non-claims preserved |
| [DP1-S Off-Repository Archive](docs/project/YIME_DUAL_PRODUCT_DP1_S_OFF_REPOSITORY_ARCHIVE_2026-09-07.md) | Actual immutable external capsule, source-unavailable PS5/PS7 reopen, idempotent verification reuse and explicit physical-durability non-claims |
