...
func main() {
	...
	config := connectedspeech.DefaultThirdToneStage5CConfig(root)
	if *outputDir != "" {
	...
	manifest, err := connectedspeech.RunThirdToneStage5CRuntime(config)
	if err != nil {
		fatal(err)
	}
	data, _ := json.MarshalIndent(manifest.Summary, "", "  ")
