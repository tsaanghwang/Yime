...
func (ime *IME) ensureToolHubManifest() (string, error) {
	...
	trainerPath := ime.trainerToolPath()
	...
	if trainerPath == "" || lexiconManagerPath == "" || reverseLookupToolPath == "" || systemLexiconAuditPath == "" || lexiconPromotionScanPath == "" || blocklistManagerPath == "" || settingsToolPath == "" || diagnosticsToolPath == "" || layoutDesignerPath == "" {
		return "", os.ErrNotExist
	}
	for _, toolPath := range []string{trainerPath, lexiconManagerPath, reverseLookupToolPath, systemLexiconAuditPath, lexiconPromotionScanPath, blocklistManagerPath, settingsToolPath, diagnosticsToolPath, layoutDesignerPath} {
