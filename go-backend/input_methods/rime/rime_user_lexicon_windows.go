...
try {
  ...
  if (-not (Test-Path -LiteralPath $sourcePath)) {
    Set-Content -LiteralPath $sourcePath -Encoding UTF8 -Value ("# PIME Yime user phrases" + [Environment]::NewLine + "# format: phrase<TAB>numeric-tone-pinyin<TAB>weight" + [Environment]::NewLine + "# example: 中国" + [char]9 + "zhong1 guo2" + [char]9 + "1000000")
  }
  Add-Content -LiteralPath $sourcePath -Encoding UTF8 -Value ($phrase + [char]9 + $pinyin + [char]9 + "1000000")

  $rimeLexiconPath = Join-Path $UserDir "custom_phrase.txt"
  if (-not (Test-Path -LiteralPath $rimeLexiconPath)) {
    Set-Content -LiteralPath $rimeLexiconPath -Encoding UTF8 -Value ("# Generated by PIME Yime from yime_user_phrases.txt" + [Environment]::NewLine + "# format: phrase<TAB>code<TAB>weight")
  }
  Add-Content -LiteralPath $rimeLexiconPath -Encoding UTF8 -Value ($phrase + [char]9 + $code + [char]9 + "1000000")
